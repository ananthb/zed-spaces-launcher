...
3. Create an annotated tag and push it:
   ...
   ```
4. The [release workflow](.github/workflows/release.yml) runs automatically on tag push. It builds Linux (tarball + AppImage) and macOS (DMG) artifacts, generates checksums, signs them with cosign, and creates a GitHub release.
...
API reference docs under `docs/api/` are generated from godoc comments using [gomarkdoc](https://github.com/princjef/gomarkdoc). The [docs workflow](.github/workflows/docs.yml) regenerates and deploys them on push to `main`.
